...
public class PlayerController : MonoBehaviour
{
    ...
    // Track which tiles are being held per lane
    ...
    public float GetLaneX(int lane)  => zoneRoots != null && lane < zoneRoots.Length ? zoneRoots[lane].transform.position.x : 0f;
    ...
    void BuildHitZones()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            bgSR.sprite = MakeSolid(new Color(0.06f,0.06f,0.11f,1f)); bgSR.sortingOrder = 20;
            bgSR.material = new Material(Shader.Find("Sprites/Default"));
            bgGO.transform.localScale = new Vector3(boxW, boxH, 1f);
            zoneBGs[i] = bgSR;

            var ringGO = new GameObject("Ring"); ringGO.transform.SetParent(root.transform, false);
            ringGO.transform.localPosition = Vector3.zero;
            var ringSR = ringGO.AddComponent<SpriteRenderer>();
            ringSR.sprite = MakeRing(col); ringSR.sortingOrder = 21;
            ...
            glowSR.sprite = MakeGlow(col); glowSR.sortingOrder = 19;
            ...
            tmp.text=activeLabels[i]; tmp.fontSize=4.2f; tmp.fontStyle=FontStyles.Bold;
            tmp.alignment=TextAlignmentOptions.Center; tmp.color=new Color(1f,1f,1f,0.92f);
            tmp.sortingOrder=22;
            zoneLabels[i] = tmp;
        }
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < zoneRoots.Length; i++)
        {
            ...
            if (zoneRings[i]) zoneRings[i].color = new Color(col.r*0.65f,col.g*0.65f,col.b*0.65f,0.75f+Mathf.Sin(t*1.2f)*0.12f);
            if (zoneGlows[i]) zoneGlows[i].color = new Color(col.r,col.g,col.b,0.07f+Mathf.Sin(t)*0.03f);
    ...
    IEnumerator FlashZone(int i)
    {
        ...
        while (t < 0.18f)
        {
            ...
            if (zoneRings[i]) zoneRings[i].color = new Color(col.r,col.g,col.b,Mathf.Lerp(1f,0.75f,p));
            if (zoneGlows[i]) zoneGlows[i].color = new Color(col.r,col.g,col.b,Mathf.Lerp(0.6f,0.07f,p));
            if (zoneLabels[i]) zoneLabels[i].color = Color.Lerp(Color.white,new Color(1f,1f,1f,0.90f),p);
            yield return null;
        }
    }

    static Sprite MakeSolid(Color col){var tex=new Texture2D(4,4);var px=new Color[16];for(int i=0;i<16;i++)px[i]=col;tex.SetPixels(px);tex.Apply();return Sprite.Create(tex,new Rect(0,0,4,4),new Vector2(.5f,.5f),4f);}
    static Sprite MakeGlow(Color col){int w=64,h=32;var tex=new Texture2D(w,h,TextureFormat.RGBA32,false);tex.filterMode=FilterMode.Bilinear;var px=new Color[w*h];for(int y=0;y<h;y++)for(int x=0;x<w;x++){float fx=x/(float)(w-1),fy=y/(float)(h-1);float dx=Mathf.Abs(fx-.5f)*2f,dy=Mathf.Abs(fy-.5f)*2f;float a=Mathf.Clamp01(1f-Mathf.Sqrt(dx*dx*.7f+dy*dy));px[y*w+x]=new Color(col.r,col.g,col.b,a*a*0.5f);}tex.SetPixels(px);tex.Apply();return Sprite.Create(tex,new Rect(0,0,w,h),new Vector2(.5f,.5f),32f);}
    static Sprite MakeRing(Color col){int w=64,h=24;var tex=new Texture2D(w,h,TextureFormat.RGBA32,false);tex.filterMode=FilterMode.Bilinear;var px=new Color[w*h];for(int y=0;y<h;y++)for(int x=0;x<w;x++){float fx=x/(float)(w-1),fy=y/(float)(h-1);float e=Mathf.Min(fx,1f-fx,fy,1f-fy)*7f;float ring=Mathf.Clamp01(1f-Mathf.Abs(e-.4f)*5.5f);px[y*w+x]=new Color(col.r,col.g,col.b,ring*0.95f);}tex.SetPixels(px);tex.Apply();return Sprite.Create(tex,new Rect(0,0,w,h),new Vector2(.5f,.5f),32f);}
