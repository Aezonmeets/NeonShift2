...
public class GameManager : MonoBehaviour
{
    ...
    void TryPlayMusic()
    {
        // Look for AudioClip named after the mode in Resources folder
        // User should place audio files in Assets/Resources/Music/
        // Named: Easy.mp3, Medium.mp3, Hard.mp3, Endless.mp3
    ...
    // ── UI BUILD (matches Figma design) ───────────────────────────────────
    void BuildUI()
    {
        var cgo=new GameObject("_Canvas");
        var cv=cgo.AddComponent<Canvas>();cv.renderMode=RenderMode.ScreenSpaceOverlay;cv.sortingOrder=20;
        ...
        // Mode badge bottom-left (like Figma "CYBER DRIFT - OST / DIFFICULTY: EXPERT")
        var modeLbl=T(cgo,"NEON SHIFT",14,new Vector2(16,50),A(0,0),A(0,0),TextAlignmentOptions.BottomLeft);
        modeLbl.color=CYAN;modeLbl.fontStyle=FontStyles.Bold;
        ...
        // ── GAME OVER PANEL (matches Figma Game Over screen) ─────────────
        goPanel=Panel(cgo,new Color(.02f,.02f,.06f,.97f));

        // "GAME OVER" — big red italic like Figma
        var goTitle=T(goPanel,"GAME OVER",88,new Vector2(0,200),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        goTitle.color=new Color(1f,.1f,.2f);goTitle.fontStyle=FontStyles.Bold|FontStyles.Italic;

        // Stats row — SCORE | ACCURACY | MAX COMBO
        var sLbl=T(goPanel,"SCORE",16,new Vector2(-220,80),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        sLbl.color=new Color(1f,.85f,.2f,.8f);sLbl.fontStyle=FontStyles.Bold;
        goScore=T(goPanel,"0",56,new Vector2(-220,30),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        goScore.color=new Color(1f,.85f,.2f);goScore.fontStyle=FontStyles.Bold;

        var aLbl=T(goPanel,"ACCURACY",16,new Vector2(0,80),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        aLbl.color=new Color(.1f,1f,.6f,.8f);aLbl.fontStyle=FontStyles.Bold;
        goAcc=T(goPanel,"0.0%",56,new Vector2(0,30),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        goAcc.color=new Color(.1f,1f,.6f);goAcc.fontStyle=FontStyles.Bold;

        var cLbl=T(goPanel,"MAX COMBO",16,new Vector2(220,80),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        cLbl.color=new Color(.4f,.7f,1f,.8f);cLbl.fontStyle=FontStyles.Bold;
        goCombo=T(goPanel,"x0",56,new Vector2(220,30),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        goCombo.color=new Color(.4f,.7f,1f);goCombo.fontStyle=FontStyles.Bold;

        // Dividers between stats
        NeonLine(goPanel,new Vector2(-110,55),new Vector2(-110,-10),CYAN,.5f);
        NeonLine(goPanel,new Vector2(110,55),new Vector2(110,-10),CYAN,.5f);

        // Buttons — PLAY AGAIN (cyan border) | MENU (dark)
        NeonBtn(goPanel,"PLAY AGAIN",CYAN,new Vector2(-110,-100),()=>Restart());
        NeonBtn(goPanel,"MENU",new Color(.4f,.5f,.6f),new Vector2(110,-100),()=>MainMenu());
        goPanel.SetActive(false);

        // ── PAUSE PANEL (matches Figma Pause screen) ─────────────────────
        pausePanel=Panel(cgo,new Color(.01f,.02f,.08f,.93f));

        // Cyan top accent line
        NeonLine(pausePanel,new Vector2(-640,358),new Vector2(640,358),CYAN,1f);

        var pauseTitle=T(pausePanel,"PAUSED",90,new Vector2(0,210),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Center);
        pauseTitle.color=CYAN;pauseTitle.fontStyle=FontStyles.Bold;
        ...
        // Pause buttons stacked — RESUME TRACK (cyan), RESTART TRACK (green), QUIT TO MENU (dark)
        ...
        // Update live score in pause via coroutine trick
        ...
        // Mode badge bottom left (like Figma)
        T(pausePanel,"NEON SHIFT",13,new Vector2(16,-330),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Left)
            .color=CYAN;
        T(pausePanel,"DIFFICULTY: "+currentMode.ToString().ToUpper(),10,new Vector2(16,-346),A(.5f,.5f),A(.5f,.5f),TextAlignmentOptions.Left)
            .color=ModeColors[(int)currentMode];
    ...
    // Neon bordered button (Game Over style)
    void NeonBtn(GameObject p,string lbl,Color col,Vector2 pos,UnityEngine.Events.UnityAction cb)
    {
        ...
        var rt=go.AddComponent<RectTransform>();rt.anchorMin=rt.anchorMax=A(.5f,.5f);
        rt.anchoredPosition=pos;rt.sizeDelta=new Vector2(200,54);
        ...
        var tgo=Label(go,lbl,26,col);
    }

    // Full-width pause button with left accent bar (like Figma)
    void PauseBtn(GameObject p,string lbl,Color col,Vector2 pos,UnityEngine.Events.UnityAction cb)
    {
        ...
        AddBorder(go,col,.5f);
        // Left accent bar (4px wide — matches Figma)
        ...
        bar.AddComponent<Image>().color=new Color(col.r,col.g,col.b,.9f);
        var btn=go.AddComponent<Button>();btn.targetGraphic=img;btn.onClick.AddListener(cb);
        Label(go,lbl,22,col);
    }
    ...
    void AddBorder(GameObject go,Color col,float alpha)
    {
        ...
        // Use outline via a simple Image with sprite (just coloured border lines)
        var O=go.AddComponent<Outline>();O.effectColor=new Color(col.r,col.g,col.b,alpha);O.effectDistance=new Vector2(1,1);
    }
